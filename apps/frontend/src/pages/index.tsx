...
const LandingPage = () => {
  ...
  // In the LandingPage component, replace the existing useEffect with this
  useEffect(() => {
    // Create stylesheet for the carousel animation
    if (isClient) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (!isPaused) {
        setActiveSlide(prevSlide => (prevSlide + 1) % thumbnails.length);
      }
    ...
    <div className="bg-[#0f0f0f] min-h-screen text-white">
      ...
      <section id="carousel" className="py-16 px-8 bg-[#121212] relative overflow-hidden">
        ...
        >
          ...
          {/* Replace the entire Carousel section with this */}
          <motion.div
            variants={slideUp}
          ...
          >
            ...
            >
              ...
              {thumbnails.map((thumb, index) => (
                <div
                  key={`original-${index}`}
                  className="min-w-[25%] px-2 flex-shrink-0"
                >
              ...
              {/* Second set of thumbnails (duplicate) */}
              {thumbnails.map((thumb, index) => (
                <div
                  key={`duplicate-${index}`}
                  className="min-w-[25%] px-2 flex-shrink-0"
